...
```{r fit-failures, eval=FALSE}
# Option 1: Exclude failed fits (default)
result <- power_analysis(fits, effect_size = 2, n_per_group = 6,
                         find = "power", on_fit_failure = "exclude")

# Option 2: Use lognormal fallback
result <- power_analysis(fits, effect_size = 2, n_per_group = 6,
                         find = "power", on_fit_failure = "lognormal")

# Option 3: Bootstrap from empirical data (not yet implemented)
# result <- power_analysis(fits, ..., on_fit_failure = "empirical")
```

**Recommendation**: Start with `"exclude"` to see how many peptides fail. If the failure rate is high (>10%), consider `"lognormal"` fallback.
