...
- **Right**: Mean abundance vs NA rate with dataset-level correlation. A negative correlation (shown in subtitle) indicates low-abundance peptides have more missing values - the hallmark of MNAR in proteomics
...
peppwR detects MNAR by correlating mean abundance with NA rate across all peptides. This dataset-level metric is shown automatically when you print a fits object and in the `plot_missingness()` visualization.
...
A strong negative correlation indicates that low-abundance peptides have more missing values, consistent with detection-limit-driven MNAR.
...
When `include_missingness = TRUE`, simulations incorporate each peptide's observed NA rate, providing power estimates that reflect what you'd actually observe in your experiment.
