...
All benchmarks use `distributions = "continuous"` which fits distributions
appropriate for continuous abundance data (gamma, lognormal, normal, etc.).

```{r benchmark-fitting, cache=TRUE, warning=FALSE}
...
```{r benchmark-aggregate, cache=TRUE, warning=FALSE}
set.seed(123)
...
```{r prepare-fits, cache=TRUE, warning=FALSE}
...
```{r benchmark-perpeptide-peptides, cache=TRUE, warning=FALSE}
set.seed(123)
...
```{r benchmark-perpeptide-nsim, cache=TRUE, warning=FALSE}
set.seed(123)
...
```{r test-speed, cache=TRUE, warning=FALSE}
set.seed(123)
...
```{r benchmark-missingness, cache=TRUE, warning=FALSE}
set.seed(123)
...
```{r benchmark-fdr, cache=TRUE, warning=FALSE}
set.seed(123)
...
FDR mode is more expensive because each simulation iteration requires:
1. Assigning peptides to null/alternative
2. Simulating all peptides together
3. Applying BH correction across all p-values

### FDR Scaling with Peptide Count

```{r benchmark-fdr-scaling, cache=TRUE, warning=FALSE}
set.seed(123)
...
```{r benchmark-plots, cache=TRUE, warning=FALSE}
set.seed(42)
...
```{r benchmark-heatmap, cache=TRUE, warning=FALSE}
set.seed(123)
