...
- `"continuous"` (default) - gamma, normal, lognormal, etc. for abundance data
- `"counts"` - includes negative binomial for count-based quantification
- `"all"` - tries all distributions with auto-detection
...
Before running power analysis, check that the fitted distributions reasonably match your data:
...
The density overlay shows observed histograms (blue) with fitted density curves (red). Good fits show the curve closely tracking the histogram shape.
...
In QQ plots, points should fall along the diagonal red line. Deviations indicate the fitted distribution doesn't fully capture the data's characteristics:
- Points curving above the line at the right indicate heavier right tails in the data
- S-shaped patterns suggest the wrong distribution family

If fits look poor, consider:
- Using different candidate distributions
- Checking for outliers or data quality issues
- Using `on_fit_failure = "empirical"` in power analysis to bootstrap from raw data

## A Note on Sample Size and Distribution Detection

With small pilot datasets (n=4 per group = 8 observations per peptide), the
"best" distribution may not match the true underlying distribution. Our
synthetic data was generated from Gamma distributions, but with small samples
you may see Pareto or Skew Normal selected as "best."

Let's verify that with larger samples, the correct distribution is identified:

```{r sample-size-sanity-check}
set.seed(42)
...
With 30 samples per group, the Gamma distribution is correctly identified as
best fit.

**Takeaway:** For small pilot studies, interpret the "best fit" as "best among
candidates given limited data" rather than definitive truth. The fitted
parameters are still useful for power simulation regardless of which
